...
const log: debug.Debugger = debug('meshage').extend('nats');
...
class NatsMeshBackend extends MeshBackendBase {
  ...
  public get subscriptionIds(): string[] {
    return [
      ...new Set([
        ...this.partitionSubscriptionIds,
        ...Object.keys(this.subscriptions).filter((subscriptionId: string) =>
          subscriptionId.includes(this.instanceId)
        ),
      ]),
    ];
  }
  ...
  public async unregister(subject: string): Promise<void> {
    ...
    for (const subscriptionSubject of Object.keys(this.subscriptions).filter(
      (subscriptionSubject: string) =>
        subscriptionSubject.indexOf(`${subject}-`) === 0
    )) {
  ...
  protected async doSend<T>(
    ...
    const localLog: debug.Debugger = log.extend(
      `send.${envelope.header.subject}.${envelope.header.name}`
    );
    localLog('%o', envelope);
    if (this.hasReceivedSubscriptionIds) {
      // has received subscription information from other nodes
      if (
        this.subscriptionIds.filter(
          (s: string) => s.indexOf(`${envelope.header.subject}-`) === 0
        ).length === 0
      ) {
    ...
    if (broadcast) {
      ...
      return new Promise<T>(
        async (resolve: (value: T) => void, reject: (error: Error) => void) => {
          const replies: T[] = [];
          const replySubscription = await this.natsConnection.subscribe(
            replySubject,
            (err: Error, msg: Msg) => {
              if (err) {
                reject(err);
      ...
      const msg: Msg = await this.natsConnection.request(
        natsSubjectToUse,
        options.timeout || 30000,
        JSON.stringify(envelope)
      );
      localLog.extend('debug')(
        'Received reply to %o - %o',
        envelope.header,
        msg
      );
      try {
  ...
  protected async doRegistrations(): Promise<void> {
    ...
    try {
      await this.initNatsConnection();
      const toRegister: MeshSubjectHandlerRegistration[] =
        this.allHandlers.filter(
          (registration: MeshSubjectHandlerRegistration) => {
            return !registration.registered;
          }
        );
      for (const registration of toRegister) {
        registration.registered = true;
        const msgCallback: MsgCallback = async (
          err: NatsError | null,
          msg: Msg
        ) => {
          // tslint:disable-next-line:no-unsafe-any
          const subjectMessageEnvelope: SubjectMessageEnvelope = JSON.parse(
            msg.data
          );
          // tslint:disable-next-line:no-any
          await this.invokeHandler(
            subjectMessageEnvelope,
            (error: MeshError, result: any) => {
              if (msg.reply) {
                this.natsConnection.publish(
                  msg.reply,
                  JSON.stringify(error ? error.serialize() : result)
                );
              }
            }
          );
        ...
        if (!this.registeredSubjects.includes(registration.subject)) {
          ...
          await this.makeSubscription(
            `${registration.subject}-broadcast`,
            msgCallback
          );
          await this.makeSubscription(
            `${registration.subject}-queue-group`,
            msgCallback,
            { queue: `${registration.subject}-qg` }
          );
        }
        await this.makeSubscription(
          `${registration.subject}-${registration.messageName}-subid-${this.instanceId}`,
          msgCallback
        );
      }
    } catch (err) {
  ...
  private async makeSubscription(
    ...
    if (!this.subscriptions[name]) {
      this.subscriptions[name] = await this.natsConnection.subscribe(
        name,
        msgCallback,
        opts
      );
  ...
  private async initNatsConnection(): Promise<Client> {
    ...
    while (!this.natsConnection && attempts > 0) {
      ...
      this.register(
        SUBJECT_NATS_MONITOR,
        SUBJECT_NATS_MONITOR_SUBSCRIPTIONS_MESSAGE_NAME,
        (msg: { subscriptions: string[] }) => {
...
function reportSubscriptions(mesh: Mesh, monitorUrl: string) {
  ...
  const interval: NodeJS.Timer = setInterval(async () => {
    ...
    try {
      ...
      // tslint:disable-next-line:no-unsafe-any
      const subs: NatsMonitorInfo = await fetch(monitorUrl, {
        headers: { accept: 'application/json' },
      }).then((res: Response) => res.json());
      const subscriptions: string[] = subs.connections.reduce(
        (nodeSubs: string[], conn: NatsSubscriptionInfo) => {
          nodeSubs.push(
            ...(conn.subscriptions_list || []).filter((sub: string) =>
              /-subid-/.test(sub)
            )
          );
          return nodeSubs;
      ...
      await mesh.subject(SUBJECT_NATS_MONITOR).broadcast({
